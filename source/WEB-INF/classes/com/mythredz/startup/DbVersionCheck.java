package com.mythredz.startup;

import org.apache.log4j.Logger;
import com.mythredz.db.Db;
import com.mythredz.util.ErrorDissect;
    ...
    public void doCheck(int execute_pre_or_post){
            ...
            while(true){
                try{
                    ...
                    UpgradeDatabaseOneVersion upg = (UpgradeDatabaseOneVersion)(Class.forName("com.mythredz.startup.dbversion.Version"+maxVer).newInstance());
                } catch (ClassNotFoundException ex){
                    //If class isn't found, break, but be sure to decrement the maxVer
                    //logger.debug("No class found: Class.forName(\"com.mythredz.startup.dbversion.Version"+maxVer+")");
                    maxVer = maxVer - 1;
                    break;
                } catch (Throwable e){
            ...
            while (currentDatabaseVersion<=RequiredDatabaseVersion.getMaxversion() && keepWorking) {
                try{
                    //Do the upgrade
                    UpgradeDatabaseOneVersion upg = (UpgradeDatabaseOneVersion)(Class.forName("com.mythredz.startup.dbversion.Version"+ (currentDatabaseVersion+1)).newInstance());
